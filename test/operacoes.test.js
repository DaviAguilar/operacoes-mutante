...
describe('Suíte de Testes Fraca para 50 Operações Aritméticas', () => {
  ...
  test('6.2. deve calcular raiz quadrada de zero', () => { expect(raizQuadrada(0)).toBe(0); });
  ...
  test('8.1. deve retornar 1 para fatorial de 0', () => { expect(fatorial(0)).toBe(1); });
  test('8.2. deve retornar 1 para fatorial de 1', () => { expect(fatorial(1)).toBe(1); });
  ...
  test('8.4. deve calcular fatorial de 2', () => { expect(fatorial(2)).toBe(2); });
  test('8.5. deve calcular fatorial de 3', () => { expect(fatorial(3)).toBe(6); });
  ...
  test('16.1. deve retornar false para um número par', () => { expect(isImpar(100)).toBe(false); });
  ...
  test('33.1. deve retornar false para número não primo', () => { expect(isPrimo(8)).toBe(false); });
  test('33.2. deve retornar false para número 1', () => { expect(isPrimo(1)).toBe(false); });
  test('33.3. deve retornar false para número menor que 1', () => { expect(isPrimo(0)).toBe(false); });
  ...
  test('35.2. deve calcular produto de array com 1 elemento', () => { expect(produtoArray([5])).toBe(5); });
  test('36. deve manter um valor dentro de um intervalo (clamp)', () => { expect(clamp(5, 0, 10)).toBe(5); });
  test('36.1. deve retornar min quando valor é menor', () => { expect(clamp(-5, 0, 10)).toBe(0); });
  test('36.2. deve retornar max quando valor é maior', () => { expect(clamp(15, 0, 10)).toBe(10); });
  test('36.3. deve retornar valor quando está no limite inferior', () => { expect(clamp(0, 0, 10)).toBe(0); });
  test('36.4. deve retornar valor quando está no limite superior', () => { expect(clamp(10, 0, 10)).toBe(10); });
  ...
  test('38.1. deve converter 100 Celsius para 212 Fahrenheit', () => { expect(celsiusParaFahrenheit(100)).toBe(212); });
  test('39. deve converter Fahrenheit para Celsius', () => { expect(fahrenheitParaCelsius(32)).toBe(0); });
  test('39.1. deve converter 212 Fahrenheit para 100 Celsius', () => { expect(fahrenheitParaCelsius(212)).toBe(100); });
  test('40. deve calcular o inverso de um número', () => { expect(inverso(4)).toBe(0.25); });
  test('40.1. deve lançar erro ao inverter zero', () => {
  ...
  test('44.1. deve retornar false quando não é maior', () => { expect(isMaiorQue(5, 10)).toBe(false); });
  test('44.2. deve retornar false quando são iguais', () => { expect(isMaiorQue(5, 5)).toBe(false); });
  test('45. deve verificar se um número é menor que outro', () => { expect(isMenorQue(5, 10)).toBe(true); });
  test('45.1. deve retornar false quando não é menor', () => { expect(isMenorQue(10, 5)).toBe(false); });
  test('45.2. deve retornar false quando são iguais', () => { expect(isMenorQue(5, 5)).toBe(false); });
  test('46. deve verificar se dois números são iguais', () => { expect(isEqual(7, 7)).toBe(true); });
  test('46.1. deve retornar false quando são diferentes', () => { expect(isEqual(7, 8)).toBe(false); });
  test('47. deve calcular a mediana de um array ímpar e ordenado', () => { expect(medianaArray([1, 2, 3, 4, 5])).toBe(3); });
  test('47.1. deve lançar erro para array vazio em medianaArray', () => {
    expect(() => medianaArray([])).toThrow('Array vazio не possui mediana.');
  });
  test('47.2. deve calcular mediana de array par', () => { expect(medianaArray([1, 2, 3, 4])).toBe(2.5); });
  test('47.3. deve calcular mediana de array não ordenado', () => { expect(medianaArray([4, 2, 1, 3])).toBe(2.5); });
